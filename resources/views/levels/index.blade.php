<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative overflow-x-auto pb-4" style="scroll-behavior: smooth; scrollbar-width: thin; scrollbar-color: #518C4F #E5E7EB;">
                        ...
                        <div class="flex flex-col items-center relative mx-2 first:ml-0 last:mr-0">
                                ...
                                <div class="relative group">
                                    <div class="w-24 h-24 rounded-full flex items-center justify-center text-4xl shadow-lg transition-all duration-300
                                        @if ($levelStatus === 'completed')
                                            shadow-md"
                                            style="background: linear-gradient(135deg, #518C4F, #27594B); border: 5px solid #F2B950;"
                                        @elseif ($levelStatus === 'current' || $levelStatus === 'started')
                                            shadow-lg"
                                            style="background: linear-gradient(135deg, #F28729, #F2B950); border: 5px solid #fff;"
                                        @else
                                            shadow-sm"
                            ...
                            <div class="flex items-center gap-2 mt-3">
                                @foreach ($levelLessons as $subIndex => $lesson)
                                    ...
                                    <div class="relative flex flex-col items-center">
                                        <a href="{{ $subCompleted || (!$subLocked && $levelStatus !== 'locked') ? route('lessons.learn', $lesson) : '#' }}"
                                           class="block w-12 h-12 rounded-full flex items-center justify-center text-base font-bold shadow-sm transition-all duration-200 hover:scale-110
                                                @if ($subCompleted)
                                                    ...
                                                    style="background: linear-gradient(135deg, #518C4F, #27594B); color: white; border: 3px solid #F2B950;"
                                                @elseif (!$subLocked && $levelStatus !== 'locked')
                                                    shadow-sm cursor-pointer"
                                                    style="background: white; color: #27594B; border: 3px solid #F28729;"
                                                @else
                                        ...
                                        <span class="text-center mt-1 leading-tight max-14">
                                            <span class="block text-[9px] font-semibold truncate
                                                @if ($subCompleted) style="color: #518C4F;" @elseif ($subLocked) style="color: #9CA3AF;" @else style="color: #27594B;" @endif
                                    ...
                                    @if (!$loop->last)
                                        <div class="w-2 h-0.5 rounded-full" style="background: #D1D5DB; margin-top: -1.25rem;"></div>
                                    @endif
                                @endforeach
                            </div>
                        </div>
                        ...
                        @if (!$loop->last)
                            <div class="flex items-center self-center mx-1" style="margin-top: -2.5rem;">
                                <svg width="32" height="8" viewBox="0 0 32 8" fill="none">
                                    <path d="M0 4H28" stroke="{{ $levelStatus === 'locked' ? '#D1D5DB' : '#518C4F' }}" stroke-width="2" stroke-dasharray="4 3"/>
                                    <polygon points="28,1 32,4 28,7" fill="{{ $levelStatus === 'locked' ? '#D1D5DB' : '#518C4F' }}"/>
                                </svg>
                            </div>
                        @endif
                    @endforeach
            ...
            <div class="flex justify-center mt-6">
                <div class="text-center">
                    ...
                    <p class="mt-1 text-sm font-semibold" style="color: #27594B;">
